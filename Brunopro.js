...
async function starts() {
	const JjBot = new WAConnection()
	JjBot.version = [2, 2119, 6]
	JjBot.logger.level = 'warn'
	console.log(banner.string)
	console.log(color('[ BOT]','aqua'), color("BOT BY BRUNO", "yellow"))
	console.log('>', '[',color('INFO','blue'),']','Seguime en ig A @y2k_bruno')
	JjBot.on('qr', () => {
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' Escanea El Código QR De Arriba'))
	})

	fs.existsSync('./session.json') && JjBot.loadAuthInfo('./session.json')
	JjBot.on('connecting', () => {
	...
	console.log(color('> [ INFO ]', 'white'), color('Ya me conecte 👌🏻'))
		...
		await JjBot.connect({timeoutMs: 30*1000})
  fs.writeFileSync('./session.json', JSON.stringify(JjBot.base64EncodedAuthInfo(), null, '\t'))
//Banned Call
JjBot.on('CB:action,,call', async json => {
    const callerId = json[2][0][1].from;
    console.log("call dari "+ callerId)
        JjBot.sendMessage(callerId, "Sistema De Bloqueo Automático, NO LLAMES POR FAVOR", MessageType.text)
        await sleep(4000)
        await JjBot.blockUser(callerId, "add")
})
//Welkom
JjBot.on('group-participants-update', async (anu) => {
		if (!welkom.includes(anu.jid)) return
		try {
			const mdata = await JjBot.groupMetadata(anu.jid)
			console.log(anu)
			if (anu.action == 'add') {
				num = anu.participants[0]
				try {
					ppimg = await JjBot.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
				} catch {
...
┏━━━━━━━━━━━━━━━━━
┠⊷️ *Suscríbete*:
┠⊷️*https://www.youtube.com/channel/UC06Fil_A_ovzT5xgx-kRKXA
┠⊷️ *Grupo Oficial*:
┠⊷️ *https://www.instagram.com/y2k_bruno/
┗━━━━━━━━━━━━━━━━━
┏━━━━━━━━━━━━━━━━━
┠⊷️「 *DESCRIPCIÓN* 」
┗━━━━━━━━━━━━━━━━━
...
┗━━━━━━━━━━━━━━━━━`
				...
				JjBot.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
...
${promote}`
			JjBot.sendMessage(mdata.id, buff, MessageType.image, {caption : teks, contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Kntl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `PROMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
...
${demote}`
			JjBot.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {mentionedJid: [num]}, quoted: { "key": { "participant": `${numbernye}`, "remoteJid": `Ktl`, "fromMe": false, "id": "B391837A58338BA8186C47E51FFDFD4A" }, "message": { "documentMessage": { "jpegThumbnail": buff, "mimetype": "application/octet-stream", "title": `DEMOTE`, "fileLength": "36", "pageCount": 0, "fileName": `_Welcome_` }}, "messageTimestamp": "1614069378", "status": "PENDING"}})
		}
		} catch (e) {
...
JjBot.on('chat-update', async (mek) => {
	try {
		...
		const q = args.join(' ')
		const meNumber = JjBot.user.jid.split("@")[0]
		const botNumber = JjBot.user.jid
//SETTING
		...
		const totalchat = await JjBot.chats.all()
		const groupMetadata = isGroup ? await JjBot.groupMetadata(from) : ''
        ...
        const conts = mek.key.fromMe ? JjBot.user.jid : JjBot.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? JjBot.user.name : conts.notify || conts.vname || conts.name || '-'
       //SETTINGS FUNCIONES
       const apakah = ['Si','No']
       ...
       const kapankah = ['Otro día','Otra semana','Otro mes','Otro año']
        ...
        const reply = (teks) => {
            JjBot.sendMessage(from, teks, text, {quoted:mek})
        }

        const sendMess = (hehe, teks) => {
            JjBot.sendMessage(hehe, teks, text, {sendEphemeral: true})
        }

        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? JjBot.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : FxBot.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const costum = (pesan, tipe, target, target2) => {
                 JjBot.sendMessage(from, pesan, tipe, {quoted: {key: {fromMe: false, participant: `${target}`, ...(from ? {remoteJid: from}: {})}, message: {conversation: `${target2}` }}})
        }
        //FAKEH
...
'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:XL;${ucapanWaktu},;;;\nFN:${ucapanWaktu},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`,
 'jpegThumbnail': fs.readFileSync('./stik/thumb.jpeg')
        ...
        const fakethumb = (teks, yes) => {
            JjBot.sendMessage(from, teks, image, {thumbnail:fs.readFileSync('./stik/fake.jpeg'),quoted:mek,caption:yes})
        }
        const fakestatus = (teks) => {
            JjBot.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
	  ...
	  contextInfo:{
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        JjBot.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    JjBot.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
//========================================================================================================================//
  ...
  wew = fs.readFileSync('./src/foto.png')
					JjBot.sendMessage(from, wew, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Felixcrack BOT Ya esta verificado.", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('./src/botlogo.webp')} } }, caption: help(prefix) })
				  break
case prefix+ 'idioma':
JjBot.sendMessage(from, idioma(prefix, sender), text, {quoted: mek})
break
case prefix+ 'yomenu':
JjBot.sendMessage(from, toinmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'menuadmin':
JjBot.sendMessage(from, menuadmin(prefix, sender), text, {quoted: mek})
break
case prefix+ 'nsfwmenu':
if (!isNsfw) return reply(mess.nsfwoff)
JjBot.sendMessage(from, nsfwmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'desmenu':
JjBot.sendMessage(from, desmenu(prefix, sender), text, {quoted: mek})
break
case prefix+ 'versión':
case prefix+ 'version':
JjBot.sendMessage(from, version(prefix, sender), text, {quoted: mek})
break
case prefix+ 'welmenu':
JjBot.sendMessage(from, welmenu(prefix, sender), text, {quoted: mek})
break
...
case prefix+ 'gtts':
case 'tts':
if (args.length < 1) return JjBot.sendMessage(from, `Debes usar el comando correctamente:\n${prefix}tts (lengua) (texto)\nEjemplo: ${prefix}tts Buen dia abortos\n\nUse: ${prefix}ts para enumerar todos los idiomas`, text, {quoted: mek})
const gtts = require('./lib/gtts')(args[0])
if (args.length < 2) return JjBot.sendMessage(from, 'Y El Texto?', text, {quoted: mek})
dtt = body.slice(8)
ranm = getRandom('.mp3')
dtt.length > 800
? reply('¿Quieres Escribir La Biblia?')
: gtts.save(ranm, dtt, function() {
JjBot.sendMessage(from, fs.readFileSync(ranm), audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
fs.unlinkSync(ranm)
})
break
...
case prefix+ 'hidetag':
			...
			var value = args.join(' ')
			var group = await JjBot.groupMetadata(from)
			...
			JjBot.sendMessage(from, optionshidetag, text)
			break
    case prefix+ 'sticktag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await JjBot.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
            value = args.join(" ")
            var group = await JjBot.groupMetadata(from)
            var member = group['participants']
            var mem = []
           ...
           JjBot.sendMessage(from, ini_buffer, MessageType.sticker, options)
            fs.unlinkSync(file)
            } else {
		    ...
		    case prefix+ 'giftag':
                ...
                download = await JjBot.downloadMediaMessage(quoted)
               ...
               JjBot.sendMessage(from, thumb, video, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'video/gif', thumbnail: thumb })
			...
			case prefix+ 'doctag':
			  //by Dehanjing
               ...
               JjBot.sendMessage(from, fs.readFileSync(`doc.txt`), document, { contextInfo: {mentionedJid: mem }, quoted: mek, mimetype: 'text/plain' })
    ...
    case prefix+ 'contag':
            ...
            + 'END:VCARD'.trim()
            JjBot.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact, {contextInfo: {"mentionedJid": members_ids}})
            break
    case prefix+ 'totag':
            if ((isMedia && !mek.message.videoMessage || isQuotedSticker) && args.length == 0) {
            encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            ...
            ini_buffer = fs.readFileSync(file)
            JjBot.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            JjBot.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            JjBot.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
        } else{
...
case prefix+ 'closegc':
case 'closegp':
JjBot.updatePresence(from, Presence.composing)
if (!isGroup) return reply(mess.only.group)
...
JjBot.groupSettingChange (from, GroupSettingChange.messageSend, true);
reply(close)
break
case prefix+ 'opengc':
case 'opengp':
...
JjBot.groupSettingChange (from, GroupSettingChange.messageSend, false);
reply(open)
break
case prefix+ 'kick':
case prefix+ 'pafuera':
if (!isGroup) return reply(mess.only.group)
					...
					JjBot.groupRemove(from, mentioned)
					} else {
                 ...
                 case prefix+ 'pm':
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						JjBot.groupMakeAdmin(from, mentioned)
					} else {
					...
					if (!isGroup) return reply(mess.only.group)
				    if (!isBotGroupAdmins) return reply(`「❗」ESTE COMANDO SOLO SE PUEDE USAR CUANDO EL BOT ES ADMIN`)
				    linkgc = await JjBot.groupInviteCode (from)
				    yeh = `https://chat.whatsapp.com/${linkgc}\n\nLink Del Grupo *${groupName}*`
				     JjBot.sendMessage(from, yeh, text, {quoted: mek1})			       
					break
case prefix+ 'leave':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
                     setTimeout( () => {
					 JjBot.groupLeave (from) 
					}, 2000)
                     setTimeout( () => {
					 JjBot.updatePresence(from, Presence.composing) 
					 JjBot.sendMessage(from, 'Adios jotos👋', text) // ur cods
					}, 0)
                     break
                ...
                case prefix+ 'marcar2':
				 JjBot.updatePresence(from, Presence.composing) 
					...
					for (let mem of groupMembers) {
					 ...
					 JjBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✪ FX ┃ ᴮᴼᵀ ✪', text, {quoted: mek})
					break
                case prefix+ 'tagall3':
                case prefix+ 'marcar3':
				 JjBot.updatePresence(from, Presence.composing) 
				 ...
				 JjBot.updatePresence(from, Presence.composing) 
					...
					for (let mem of groupMembers) {
					 ...
					 JjBot.sendMessage(from, '𝙈𝙚𝙣𝙘𝙞𝙤𝙣 𝘿𝙚 𝙏𝙤𝙙𝙤𝙨'+teks+'✔︎', text, {quoted: mek})
					break
                case prefix+ 'tagall5':
                case prefix+ 'marcar5':
				 JjBot.updatePresence(from, Presence.composing) 
...
case prefix+ 'tovid':
    ...
    case prefix+ 'toimg':
			...
			reply(mess.wait)
			encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
			media = await JjBot.downloadAndSaveMediaMessage(encmedia)
			ran = getRandom('.png')
			exec(`ffmpeg -i ${media} ${ran}`, (err) => {
			fs.unlinkSync(media)
...
reply('En Proceso ⌛')
owgi = await JjBot.downloadAndSaveMediaMessage(ger)
      ...
      rano = getRandom('.webp')
			...
			exec(`wget ${anu.data.url} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
			  fs.unlinkSync(ranp)
				...
				JjBot.sendMessage(from, buffer, MessageType.sticker, {quoted: mek})
				fs.unlinkSync(rano)
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
  pusiimg = await axios.get('https://nekos.life/api/v2/img/pussy_jpg')
			bufpusy = await getBuffer(pusiimg.data.url)
				JjBot.sendMessage(from, bufpusy, MessageType.image, {quoted: mek})
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			JjBot.sendMessage(from, bupyuri, image, { quoted: mek })
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			JjBot.sendMessage(from, buferon, image, { quoted: mek })
			.catch(err => {
			return('Error 😔..')
			})
			break
...
if (!isNsfw) return reply(mess.nsfwoff)
			...
			bufero = await getBuffer(eroz.data.url)
			 JjBot.sendMessage(from, bufero, image, { quoted: mek })
			.catch(err => {
...
if (!isNsfw) return reply(mess.nsfwoff)
	...
	if (!isNsfw) return reply(mess.nsfwoff)
			...
			bufhtz = await getBuffer(hentaiz.data.url)
			 JjBot.sendMessage(from, bufhtz, image, {quoted: mek})
			.catch(err => {
			return('Error 😔..')
			})
			break
...
case prefix+ 'stickergif': 
    ...
    case prefix+ 's':
            ...
            const media = await  JjBot.downloadAndSaveMediaMessage(encmedia)
                ran = '666.webp'
                await ffmpeg(`./${media}`)
                .input(media)
                ...
                .on('end', function () {
                console.log('Finish')
                JjBot.sendMessage(from, fs.readFileSync(ran), MessageType.sticker, {quoted: mek})
                 fs.unlinkSync(media)
                fs.unlinkSync(ran)
                })
...
JjBot.sendMessage(from, attp2, MessageType.sticker, selepbot, {quoted: mek })
						break
...
break
       ...
       //OWNER BOT
            ...
            if (!mek.key.fromMe) return fakestatus('「 ❗ 」ESTE COMANDO SOLO PUEDE SER USADO POR MI')
            var chats = await JjBot.chats.all()
            chats.map( async ({ jid }) => {
            await JjBot.chatRead(jid)
            })
		    var teks = `\`\`\`Fueron Leídos ${chats.length} Chats !\`\`\``
	        await JjBot.sendMessage(from, teks, text, {quoted: mek})
...
reply(mess.mpv)
	...
	case prefix+ 'ytsearch':
        	...
        	return await JjBot.sendMessage(from, 'Error!', MessageType.text, dload)
    		}
    		aramat = aramas.all 
...
case prefix+ 'play':
    ...
    JjBot.sendMessage(from, options, text, { quoted: mek } )
				break
case prefix+ 'creador':
await JjBot.sendMessage(from, {displayname: "Bruno🧙‍♂️", vcard: vcard}, MessageType.contact)
break
